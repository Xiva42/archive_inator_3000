...
def write_to_sheet(sheet_obj:gspread.Spreadsheet, data:list[any], worksheet_name:str = ARCHIVE_INATOR_STORAGE_SHEET) -> int:
    ...
    archive_sheet_filter_function_tail = ["","",
                                          ...
                                          "",
                                          f"=IF(${numToSheetChar(tail_start_column+1)}$1=\"\";;IF(${numToSheetChar(tail_start_column+2)}$1=\"\";IF(${numToSheetChar(tail_start_column+1)}{row_number}=\"\";;\"X\");IF(${numToSheetChar(tail_start_column+3)}$1=\"\";IF(${numToSheetChar(tail_start_column+2)}{row_number}=\"\";;\"X\");IF(${numToSheetChar(tail_start_column+4)}$1=\"\";IF(${numToSheetChar(tail_start_column+3)}{row_number}=\"\";;\"X\");IF(${numToSheetChar(tail_start_column+4)}{row_number}=\"\";;\"X\")))))",
